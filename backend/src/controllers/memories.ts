...
import { unclassifiedMemoriesTable } from '@/db/schema.ts';
import { verifyJWT } from '@/lib/auth/auth.ts';
import { classifyMemory, restoreMemory } from '@/lib/memories.ts';
import { UnclassifiedMemory } from '@/types/types.ts';
	...
	try {
		...
		if (isMemoryCorrect) {
			try {
				...
				if (shouldRestore) {
					await restoreMemory(memory, userId);
					res.status(200).json({
